...
/*接收缓冲,最大CMD_REC_LEN个字节*/
static u8 USART1_RX_BUF[CMD_REC_LEN];
static u8 USART2_RX_BUF[CMD_REC_LEN];
/*接收状态标记：
 * bit15，	接收完成标志
 * bit14，	接收到0x0d
 * bit13~0，接收到的有效字节数目
...
void USART1_IRQHandler(void)                	
{
		...
		if((USART1_RX_STA&0x8000)==0){//接收未完成
			if(USART1_RX_STA&0x4000){//接收到了0x0d
				...
				if(ReceiveByte==0x0d)USART1_RX_STA|=0x4000;
					...
					USART1_RX_STA++;
					if(USART1_RX_STA>(CMD_REC_LEN-1))USART1_RX_STA=0;//接收数据错误,重新开始接收	  
				}
			}
		}
  }
	...
	if((USART1_RX_STA&0x8000)&&(CMD_Get_Queue!=NULL)){
		...
		USART1_RX_STA=0;
		for(int i=0; i<CMD_REC_LEN; i++)USART1_RX_BUF[i]=0;
